...
namespace _25
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var command = new Lazy<IEnumerator>(() => Console.ReadLine().ToCharArray().GetEnumerator());
            Func<long> inputProviderFactory = () =>
            {   
                if(command.Value.MoveNext())
                {
                    //Console.Write(command.Value.Current);
                    return Convert.ToInt32(command.Value.Current);
                }
                command = new Lazy<IEnumerator>(() => Console.ReadLine().ToCharArray().GetEnumerator());
                return (int)'\n';
            };
